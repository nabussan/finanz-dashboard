...
async def portfolio_listen_default(request: Request):
    ...
    first = await pool.fetchrow(
        """SELECT c.id FROM clusters c
           ...
           WHERE cv.view_name = 'portfolio' ORDER BY c.id LIMIT 1"""
    )
    if first:
...
async def portfolio_listen_page(request: Request, list_id: int, reclassify: int = 0):
    pool = await db.get_pool()

    all_lists = [dict(r) for r in await pool.fetch(
        """SELECT c.id, c.name FROM clusters c
           JOIN cluster_views cv ON cv.cluster_id = c.id
           WHERE cv.view_name = 'portfolio' ORDER BY c.id"""
    )]
